...
public class ECMMiner {
    ...
    public List<Musician> mostSocialMusicians(int k) {
        ...
        ListMultimap<String, Musician> multimap = MultimapBuilder.treeKeys().arrayListValues().build();
        ListMultimap<Integer, Musician> countMap = MultimapBuilder.treeKeys().arrayListValues().build();
        ...
        for (Musician musician : musicians) {
            ...
            for (Album album : albums) {
                List<Musician> albumMusicians = album.getFeaturedMusicians();
                for (Musician musician1 : albumMusicians) {
                    if (musician.getName() != musician1.getName()) {
                        if (multimap.isEmpty()) {
                            multimap.put(musician.getName(), musician1);
                        }
                        else if (!multimap.containsEntry(musician.getName(), musician1)) {
                            multimap.put(musician.getName(), musician1);
                        }
                        /*if (cooperateMusicians.size() == 0) {
                            cooperateMusicians.add(musician1);
                            multimap.put(musician.getName(), musician1);
                        }
                        else if (!cooperateMusicians.contains(musician1)) {
                            cooperateMusicians.add(musician1);
                            multimap.put(musician.getName(), musician1);
                        }*/
                    }
        ...
        Map<String, Collection<Musician>> musicianMultimap = multimap.asMap();
        for (String name : musicianMultimap.keySet()) {
            Collection<Musician> cooperatedMusicians = musicianMultimap.get(name);
            int size = cooperatedMusicians.size();
            countMap.put(size, nameMap.get(name));
        }
        ...
        for (Integer count : sortedKeys) {
            ...
            if (result1.size() + list.size() >= k) {
                int newAddition = k - result1.size();
                for (int i = 0; i < newAddition; i++) {
